...
async function request<T>(path: string, options: RequestInit = {}): Promise<T> { const token = typeof window !== 'undefined' ? localStorage.getItem('sfb_token') : null; const res = await fetch(`${base}${path}`, { ...options, headers: { 'Content-Type': 'application/json', ...(token ? { Authorization: `Bearer ${token}` } : {}), ...options.headers } }); const body = await res.json(); if (!res.ok) throw normalizeApiError(body, res.status); return body }
export const api = {
 ...
 async register(data: { email: string; password: string; name: string }) { if (useMock) { if (mockUsers.some(u => u.email === data.email)) throw new Error('Email đã được sử dụng'); return mockUsers[0] } return request<User>('/auth/register', { method: 'POST', body: JSON.stringify(data) }) },
